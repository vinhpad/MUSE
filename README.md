...
```bash
bash scripts/flame.sh        # run FLAME on the dataset set inside the script
...
```bash
python main.py \
  --mode flame --dataset cifar100 --n_tasks 5 --n 50 --m 10 --rnd_NM \
  ...
  --data_dir local_datasets --rnd_seed 1 --note FLAME --eval_period 1000
```